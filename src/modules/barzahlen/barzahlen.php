...
class Barzahlen extends PaymentModule
{
    ...
    {
        if (!parent::install() || !$this->registerHook('payment') || !$this->registerHook('paymentReturn') || !$this->registerHook('updateOrderStatus') || !$this->registerHook('backOfficeFooter')) {
            return false;
        }
        $this->createTables();
                ...
                if ($response != false) {
                    echo '<script type="text/javascript">
                          if (confirm("' . sprintf($this->l('For the Barzahlen plugin is a new version (%s) available. View now?'), (string) $response) . '")) {
                              window.location.href = "https://integration.barzahlen.de/en/shopsystems/prestashop";
                          }
                          </script>';
                }
            } catch (Exception $e) {
    ...
    /**
     * Get execute payment HTML.
     *
     * @global object $smarty smarty handler
     * @param object $cart customer cart
     * @return string
     */
